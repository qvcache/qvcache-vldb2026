...
template <typename T, typename TagT> class PQFlashIndex;
...
DISKANN_DLLEXPORT double calculate_recall(unsigned num_queries,
...
calculate_recall(unsigned num_queries, unsigned *gold_std, float *gs_dist,
...
merge_shards(const std::string &vamana_prefix, const std::string &vamana_suffix,
...
DISKANN_DLLEXPORT uint32_t optimize_beamwidth(
    std::unique_ptr<greator::PQFlashIndex<T, TagT>> &_pFlashIndex,
    T *tuning_sample, _u64 tuning_sample_num, _u64 tuning_sample_aligned_dim,
    uint32_t L, uint32_t nthreads, uint32_t start_bw = 2);

template <typename T, typename TagT = uint32_t>
DISKANN_DLLEXPORT bool
build_disk_index(const char *dataFilePath, const char *indexFilePath,
...
create_disk_layout(const std::string &mem_index_file,
                   const std::string &base_file, const std::string &tag_file,

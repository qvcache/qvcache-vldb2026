...
namespace greator {
template <typename T, typename TagT = uint32_t> class Index {
public:
  ...
  build(const char *filename, const size_t num_points_to_load,
  ...
  search(const T *query, const size_t K, const unsigned L, unsigned *indices,
         float *distances = nullptr);

  DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t>
  search(const T *query, const uint64_t K, const unsigned L,
  ...
  DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t>
  search(const T *query, const size_t K, const unsigned L,
   ...
   * exist in the graph */
  DISKANN_DLLEXPORT int
  insert_point(const T *point, const Parameters &parameter,
  ...
  DISKANN_DLLEXPORT int
  extract_data(T *ret_data,
               std::unordered_map<TagT, unsigned> &tag_to_location);

  DISKANN_DLLEXPORT void
  get_location_to_tag(std::unordered_map<unsigned, TagT> &ret_loc_to_tag);
...
protected:
  ...
  iterate_to_fixed_point(const T *node_coords, const unsigned Lindex,
                         const std::vector<unsigned> &init_ids,
                         std::vector<Neighbor> &expanded_nodes_info,
                         tsl::robin_set<unsigned> &expanded_nodes_ids,
                         std::vector<Neighbor> &best_L_nodes,
                         bool ret_frozen = true);
  ...
  void batch_inter_insert(unsigned n, const std::vector<unsigned> &pruned_list,
                          const Parameters &parameter,

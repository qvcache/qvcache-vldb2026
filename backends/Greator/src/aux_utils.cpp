...
void add_new_file_to_single_index(std::string index_file,
  ...
  _u64 nr, nc;
  greator::load_bin<_u64>(index_file, metadata, nr, nc, 0);
  if (nc != 1) {
  ...
  greator::save_bin<_u64>(index_file, new_meta.data(), new_meta.size(), 1, 0);
}
...
size_t calculate_num_pq_chunks(double final_index_ram_limit, size_t points_num,
  ...
  greator::cout << "Calculated num_pq_chunks :" << num_pq_chunks << std::endl;
  num_pq_chunks = num_pq_chunks <= 0 ? 1 : num_pq_chunks;
  num_pq_chunks = num_pq_chunks > dim ? dim : num_pq_chunks;
  num_pq_chunks = num_pq_chunks > MAX_PQ_CHUNKS ? MAX_PQ_CHUNKS : num_pq_chunks;

  greator::cout << "Compressing " << dim << "-dimensional data into "
...
double calculate_recall(unsigned num_queries, unsigned *gold_std,
  ...
  for (size_t i = 0; i < num_queries; i++) {
    ...
    if ((active_points_count < recall_at && !active_tags.empty()) && !printed) {
      greator::cout << "Warning: Couldn't find enough closest neighbors "
...
T *generateRandomWarmup(uint64_t warmup_num, uint64_t warmup_dim,
  ...
  T *warmup = nullptr;
  warmup_num = 100000;
  greator::cout << "Generating random warmup file with dim " << warmup_dim
  ...
  if (files.fileExists(cache_warmup_file)) {
    ...
    if (file_dim != warmup_dim || file_aligned_dim != warmup_aligned_dim) {
      ...
      stream << "Mismatched dimensions in sample file. file_dim = " << file_dim
             << " file_aligned_dim: " << file_aligned_dim
...
T *load_warmup(const std::string &cache_warmup_file, uint64_t &warmup_num,
  ...
  if (file_exists(cache_warmup_file)) {
    greator::load_aligned_bin<T>(cache_warmup_file, warmup, warmup_num,
                                 file_dim, file_aligned_dim);
    if (file_dim != warmup_dim || file_aligned_dim != warmup_aligned_dim) {
      std::stringstream stream;
      stream << "Mismatched dimensions in sample file. file_dim = " << file_dim
...
int merge_shards(const std::string &vamana_prefix,
  ...
  nnodes++;
  greator::cout << "# nodes: " << nnodes << ", max. degree: " << max_degree
                << std::endl;
  ...
  for (_u64 shard = 0; shard < nshards; shard++) {
    greator::cout << "Creating inverse map -- shard #" << shard << std::endl;
  ...
  greator::cout << "Finished computing node -> shards map" << std::endl;
  ...
  greator::cout << "Max input width: " << max_input_width
  ...
  greator::cout << "Starting merge" << std::endl;
  ...
  greator::cout << "Expected size: " << merged_index_size << std::endl;

  diskann_writer.reset();
  diskann_writer.write((char *)&merged_index_size, sizeof(uint64_t));

  greator::cout << "Finished merge" << std::endl;
  return 0;
}

template <typename T>
int build_merged_vamana_index(
    std::string base_file, greator::Metric _compareMetric,
  ...
  greator::get_bin_metadata(base_file, base_num, base_dim);

  double full_index_ram = ESTIMATE_RAM_USAGE(base_num, base_dim, sizeof(T), R);
  if (full_index_ram < ram_budget * 1024 * 1024 * 1024) {
    greator::cout << "Full index fits in RAM, building in one shot"
                  << std::endl;
    ...
    std::unique_ptr<greator::Index<T>> _pvamanaIndex =
        std::unique_ptr<greator::Index<T>>(
            new greator::Index<T>(_compareMetric, base_dim, base_num, false,
  ...
  if (single_file_index || tag_file != nullptr) {
    greator::cout << "Cannot build merged index if single_file_index is "
  ...
  for (int p = 0; p < num_parts; p++) {
    ...
    std::unique_ptr<greator::Index<T>> _pvamanaIndex =
        std::unique_ptr<greator::Index<T>>(new greator::Index<T>(
  ...
  greator::merge_shards(merged_index_prefix + "_subshard-", "_mem.index",
...
uint32_t
optimize_beamwidth(std::unique_ptr<greator::PQFlashIndex<T, TagT>> &pFlashIndex,
  ...
  while (!stop_flag) {
    ...
    greator::QueryStats *stats = new greator::QueryStats[tuning_sample_num];
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
    ...
    double lat_999 = greator::get_percentile_stats(
        stats, tuning_sample_num, 0.999f,
        [](const greator::QueryStats &stats) { return stats.total_us; });

    double mean_latency = greator::get_mean_stats(
...
void create_disk_layout(const std::string &mem_index_file,
  ...
  if (single_file_index) {
    ...
    greator::load_bin<_u64>(mem_index_file, offsets, nr, nc);
...
diskann:
  ...
  greator::cout << "medoid: " << medoid << "B" << std::endl;
  greator::cout << "max_node_len: " << max_node_len << "B" << std::endl;
  greator::cout << "nnodes_per_sector: " << nnodes_per_sector << "B"
                << std::endl;
  ...
  greator::cout << "# sectors: " << n_sectors << std::endl;
  _u64 cur_node_id = 0;
  for (_u64 sector = 0; sector < n_sectors; sector++) {
    if (sector % 100000 == 0) {
      greator::cout << "Sector #" << sector << "written" << std::endl;
    }
    ...
    for (_u64 sector_node_id = 0;
      ...
      if (nnbrs == 0) {
        greator::cout << "ERROR. Found point with no out-neighbors; Point#: "
  ...
  if (vamana_frozen_num > 0) {
    ...
    size_t nr, nc;
    if (single_file_index)
      greator::load_bin<TagT>(mem_index_file, mem_index_tags, nr, nc,
                              tags_offset);
    else
      greator::load_bin<TagT>(tag_file, mem_index_tags, nr, nc, tags_offset);
    ...
    if (single_file_index) {
      tag_bytes_written = greator::save_bin<TagT>(
          output_file, mem_index_tags.get(), nr, nc,
          output_file_meta[output_file_meta.size() - 1]);
    } else {
      greator::save_bin<TagT>(output_file + std::string(".tags"),
    ...
    if (tags_enabled) {
      ...
      if (single_file_index) {
        ...
        if (!file_exists(tag_file)) {
          greator::cout << "Static vamana index, tag file " << tag_file
  ...
  output_file_meta.push_back(output_file_meta[output_file_meta.size() - 1] +
                             tag_bytes_written);
  greator::save_bin<_u64>(output_file, output_file_meta.data(),
  ...
  greator::cout << "Output file written." << std::endl;
}

template <typename T, typename TagT>
bool build_disk_index(const char *dataPath, const char *indexFilePath,
                      const char *indexBuildParameters,
                      greator::Metric _compareMetric, bool single_file_index,
                      const char *tag_file) {
  ...
  if (final_index_ram_limit <= 0) {
    greator::cerr << "Insufficient memory budget (or string was not in right "
                     "format). Should be > 0."
                  << std::endl;
    return false;
  }
  ...
  if (indexing_ram_budget <= 0) {
    greator::cerr << "Not building index. Please provide more RAM budget"
                  << std::endl;
    return false;
  }
  ...
  greator::cout << "Starting index build: R=" << R << " L=" << L
  ...
  if (_compareMetric == greator::Metric::COSINE) {
    if (std::is_floating_point<T>::value) {
      greator::cout << "Cosine metric chosen. Normalizing vectors and "
      ...
      _compareMetric = greator::Metric::L2;
    } else {
  ...
  greator::get_bin_metadata(normalized_file_path, points_num, dim);
  ...
  greator::cout << "(Normalized, if required) file : " << normalized_file_path
                << " has: " << points_num
  ...
  greator::cout << "Generating PQ pivots with training data of size: "
  ...
  greator::cout << "Pivots generated in "
  ...
  greator::cout << "Compressed data generated and written in: "
                << std::chrono::duration<double>(end - start).count() << "s."
                << std::endl;
  start = std::chrono::high_resolution_clock::now();
  greator::build_merged_vamana_index<T>(normalized_file_path, _compareMetric,
                                        single_file_index, L, R, p_val,
                                        indexing_ram_budget, mem_index_path,
                                        medoids_path, centroids_path, tag_file);
  end = std::chrono::high_resolution_clock::now();
  greator::cout << "Vamana index built in: "
                << std::chrono::duration<double>(end - start).count() << "s."
                << std::endl;

  if (tag_file == nullptr) {
    greator::create_disk_layout<T, TagT>(
  ...
  greator::cout << "Generating warmup file with " << num_sample_points
  ...
  greator::cout << "Deleting memory index file: " << mem_index_path
                << std::endl;
  ...
  if (normalized_file_path != dataFilePath) {
    ...
    greator::cout << "Deleting normalized vector file: " << normalized_file_path
                  << std::endl;
  ...
  auto e = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> diff = e - s;
  greator::cout << "Indexing time: " << diff.count() << std::endl;
  return true;
}
...
template DISKANN_DLLEXPORT int8_t *
load_warmup<int8_t>(const std::string &cache_warmup_file, uint64_t &warmup_num,
                    uint64_t warmup_dim, uint64_t warmup_aligned_dim);
template DISKANN_DLLEXPORT uint8_t *
load_warmup<uint8_t>(const std::string &cache_warmup_file, uint64_t &warmup_num,
                     uint64_t warmup_dim, uint64_t warmup_aligned_dim);
template DISKANN_DLLEXPORT float *
load_warmup<float>(const std::string &cache_warmup_file, uint64_t &warmup_num,
                   uint64_t warmup_dim, uint64_t warmup_aligned_dim);

#ifdef EXEC_ENV_OLS
template DISKANN_DLLEXPORT int8_t *
load_warmup<int8_t>(MemoryMappedFiles &files,
                    const std::string &cache_warmup_file, uint64_t &warmup_num,
                    uint64_t warmup_dim, uint64_t warmup_aligned_dim);
template DISKANN_DLLEXPORT uint8_t *
load_warmup<uint8_t>(MemoryMappedFiles &files,
                     const std::string &cache_warmup_file, uint64_t &warmup_num,
                     uint64_t warmup_dim, uint64_t warmup_aligned_dim);
template DISKANN_DLLEXPORT float *
load_warmup<float>(MemoryMappedFiles &files,
                   const std::string &cache_warmup_file, uint64_t &warmup_num,
                   uint64_t warmup_dim, uint64_t warmup_aligned_dim);
#endif

template DISKANN_DLLEXPORT uint32_t optimize_beamwidth<int8_t, uint32_t>(
    std::unique_ptr<greator::PQFlashIndex<int8_t, uint32_t>> &pFlashIndex,
...
template DISKANN_DLLEXPORT bool build_disk_index<int8_t, uint32_t>(
    ...
    const char *indexBuildParameters, greator::Metric _compareMetric,

...
Index<T, TagT>::Index(Metric m, const size_t dim, const size_t max_points,
    ...
    : _dist_metric(m), _dim(dim), _max_points(max_points),
      _save_as_one_file(save_index_in_one_file), _dynamic_index(dynamic_index),
  ...
  if (support_eager_delete && !dynamic_index) {
    greator::cout << "ERROR: Eager Deletes must have Dynamic Indexing "
                     "enabled. Exitting."
                  << std::endl;
    exit(-1);
  }
  ...
  greator::cout << "Getting distance function for metric: "
...
template <typename T, typename TagT> Index<T, TagT>::~Index() {
  delete this->_distance;
  aligned_free(_data);
}

template <typename T, typename TagT> void Index<T, TagT>::clear_index() {
...
_u64 Index<T, TagT>::save_tags(std::string tags_file, size_t offset) {
  if (!_enable_tags) {
    greator::cout << "Not saving tags as they are not enabled." << std::endl;
    return 0;
  }
...
void Index<T, TagT>::save(const char *filename) {
  ...
  if (!_save_as_one_file) {
    ...
    greator::save_bin<_u64>(filename, cumul_bytes.data(), cumul_bytes.size(), 1,
                            0);

    greator::cout << "Saved index as one file to " << filename << " of size "
  ...
  greator::cout << "Time taken for save: " << timespan.count() << "s."
                << std::endl;
}
...
size_t Index<T, TagT>::load_tags(const std::string tag_filename,
  ...
  if (_enable_tags && !file_exists(tag_filename)) {
    greator::cerr << "Tag file provided does not exist!" << std::endl;
    throw greator::ANNException("Tag file provided does not exist!", -1,
                                __FUNCSIG__, __FILE__, __LINE__);
  }

  if (!_enable_tags) {
    greator::cout << "Tags not loaded as tags not enabled." << std::endl;
    return 0;
  }
  ...
  greator::cout << "Tags loaded." << std::endl;
  delete[] tag_data;
...
size_t Index<T, TagT>::load_data(std::string filename, size_t offset) {
  ...
  size_t file_dim, file_num_points;
  greator::get_bin_metadata(filename, file_num_points, file_dim, offset);
  ...
  copy_aligned_data_from_file<T>(std::string(filename), _data, file_num_points,
                                 file_dim, _aligned_dim, offset);
  return file_num_points;
}
...
void Index<T, TagT>::load(const char *filename) {
  ...
  if (!_save_as_one_file) {
    ...
    greator::load_bin<_u64>(index_file, file_offset_data, nr, nc, 0);
  ...
  if (data_file_num_pts != graph_num_pts ||
    ...
    stream << "ERROR: When loading index, loaded " << data_file_num_pts
           << " points from datafile, " << graph_num_pts << " from graph, and "
           << tags_file_num_pts << " tags, with num_frozen_pts being set to "
  ...
  reposition_frozen_point_to_end();
  greator::cout << "Num frozen points:" << _num_frozen_pts << " _nd: " << _nd
                << " _ep: " << _ep
...
size_t Index<T, TagT>::load_graph(std::string filename,
  ...
  greator::cout << "Loading vamana index " << filename << "..." << std::flush;
  ...
  if (_max_points < (expected_num_points - _num_frozen_pts)) {
    greator::cout << "Number of points in data: " << expected_num_points
  ...
  while (bytes_read != expected_file_size) {
    ...
    if (k == 0) {
      greator::cerr << "ERROR: Point found with no out-neighbors, point#"
                    << nodes << std::endl;
    }
    //      if (in.eof())
    //        break;
  ...
  greator::cout << "done. Index has " << nodes << " nodes and " << cc
...
int Index<T, TagT>::get_vector_by_tag(TagT &tag, T *vec) {
  ...
  if (_tag_to_location.find(tag) == _tag_to_location.end()) {
    greator::cout << "Tag " << tag << " does not exist" << std::endl;
    return -1;
  }
...
const T *Index<T, TagT>::get_vector_by_tag(const TagT &tag) {
  std::shared_lock<std::shared_timed_mutex> lock(_tag_lock);
  if (_tag_to_location.find(tag) == _tag_to_location.end()) {
    greator::cout << "Tag " << tag << " does not exist in the index."
                  << std::endl;
    return nullptr;
  } else {
    unsigned location = _tag_to_location[tag];
    ...
    if (best_L_nodes[k].flag) {
      ...
      if (_dynamic_index) {
        ...
        for (unsigned m = 0; m < _final_graph[n].size(); m++) {
          if (_final_graph[n][m] >= _max_points + _num_frozen_pts) {
            greator::cerr << "Wrong id found: " << _final_graph[n][m]
                          << std::endl;
            throw greator::ANNException(std::string("Wrong id found") +
      ...
      for (unsigned m = 0; m < des.size(); ++m) {
        ...
        if (inserted_into_pool.find(id) == inserted_into_pool.end()) {
          ...
          if ((m + 1) < des.size()) {
            auto nextn = des[m + 1];
            greator::prefetch_vector((const char *)_data +
                                         _aligned_dim * (size_t)nextn,
...
void Index<T, TagT>::prune_neighbors(const unsigned location,
  ...
  if (pool.size() == 0) {
    throw greator::ANNException("Pool passed to prune_neighbors is empty", -1);
  }
...
void Index<T, TagT>::batch_inter_insert(
  ...
  for (auto des : pruned_list) {
    ...
    if (des > _max_points)
      greator::cout << "error. " << des << " exceeds max_pts" << std::endl;
...
void Index<T, TagT>::link(Parameters &parameters) {
  ...
  if (NUM_SYNCS < 40)
    NUM_SYNCS = 40;
  greator::cout << "Number of syncs: " << NUM_SYNCS << std::endl;
  ...
  std::vector<greator::Neighbor> pool, tmp;
...
#pragma omp parallel for schedule(dynamic, 65536)
      ...
      if ((sync_num * 100) / NUM_SYNCS > progress_counter) {
        greator::cout.precision(4);
        greator::cout << "Completed  (round: " << rnd_no
...
// This code should only be available in the "build" DLL.
...
#endif
    if (_nd > 0) {
      greator::cout << "Completed Pass " << rnd_no << " of data using L=" << L
                    << " and alpha=" << parameters.Get<float>("alpha")
                    << ". Stats: ";
      greator::cout << "search+prune_time=" << total_sync_time
  ...
  if (_nd > 0) {
    greator::cout << "Starting final cleanup.." << std::flush;
  }
...
#pragma omp parallel for
  ...
  greator::cout << "Prune time : " << timer.elapsed() / 1000 << "ms"
                << std::endl;
  ...
  if (min > max)
    min = max;
  if (_nd > 0) {
    greator::cout << "Index built with degree: max:" << max
...
void Index<T, TagT>::build(const char *filename,
  ...
  if (!file_exists(filename)) {
    greator::cerr << "Data file " << filename
  ...
  if (filename == nullptr) {
    greator::cout << "Starting with an empty index." << std::endl;
    _nd = 0;
  } else {
    greator::get_bin_metadata(filename, file_num_points, file_dim);
    ...
    greator::cout << "Loading only first " << num_points_to_load
                  << " from file.. " << std::endl;
    ...
    if (_enable_tags) {
      if (tag_filename == nullptr) {
        ...
        if (file_exists(tag_filename)) {
          greator::cout << "Loading tags from " << tag_filename
                        << " for vamana index build" << std::endl;
          TagT *tag_data = nullptr;
          size_t npts, ndim;
          greator::load_bin(tag_filename, tag_data, npts, ndim);
          if (npts != num_points_to_load) {
            std::stringstream sstream;
            sstream << "Loaded " << npts << " tags instead of expected number: "
                    << num_points_to_load;
            greator::cerr << sstream.str() << std::endl;
            throw greator::ANNException(sstream.str(), -1, __FUNCSIG__,
                                        __FILE__, __LINE__);
          }
          for (size_t i = 0; i < npts; i++) {
          ...
          greator::cerr << "Tag file " << tag_filename
                        << " does not exist. Exiting..." << std::endl;
          throw greator::ANNException(std::string("Tag file") + tag_filename +
                                          " does not exist",
                                      -1, __FUNCSIG__, __FILE__, __LINE__);
        }
      }
...
Index<T, TagT>::search(const T *query, const size_t K, const unsigned L,
...
template <typename T, typename TagT>
std::pair<uint32_t, uint32_t>
Index<T, TagT>::search(const T *query, const uint64_t K, const unsigned L,
...
template <typename T, typename TagT> size_t Index<T, TagT>::get_num_points() {
  return _nd;
}

template <typename T, typename TagT> T *Index<T, TagT>::get_data() {
  if (_num_frozen_pts > 0) {
    T *ret_data = nullptr;
...
template <typename T, typename TagT> int Index<T, TagT>::enable_delete() {
  assert(_enable_tags);
...
template <typename T, typename TagT> void Index<T, TagT>::release_location() {
...
int Index<T, TagT>::eager_delete(const TagT tag, const Parameters &parameters,
  ...
  if (_lazy_done && (!_data_compacted)) {
    greator::cout << "Lazy delete requests issued but data not consolidated, "
...
template <typename T, typename TagT> void Index<T, TagT>::update_in_graph() {
...
size_t Index<T, TagT>::consolidate_deletes(const Parameters &parameters) {
  ...
  greator::cout << "Inside Index::consolidate_deletes()" << std::endl;
...
#pragma omp parallel for schedule(dynamic)
  for (_s64 block = 0; block < total_blocks; ++block) {
    ...
    for (_s64 i = block * block_size; i < (_s64)((block + 1) * block_size) &&
...
template <typename T, typename TagT> void Index<T, TagT>::compact_data() {
  if (!_dynamic_index)
    return;
  ...
  std::vector<unsigned> new_location =
      std::vector<unsigned>(_max_points + _num_frozen_pts, (_u32)_max_points);
  ...
  if (_delete_set.find(_ep) != _delete_set.end()) {
    ...
    if (_ep == old_ep) {
      greator::cerr << "ERROR: Did not find a replacement for start node."
                    << std::endl;
      throw greator::ANNException(
...
template <typename T, typename TagT> int Index<T, TagT>::reserve_location() {
  LockGuard guard(_change_lock);
  if (_nd >= _max_points) {
    return -1;
  }
...
void Index<T, TagT>::reposition_point(unsigned old_location,
  ...
  memcpy((void *)(_data + (size_t)_aligned_dim * new_location),
         _data + (size_t)_aligned_dim * old_location, sizeof(T) * _aligned_dim);
...
void Index<T, TagT>::resize(uint32_t new_max_points) {
  ...
  auto stop = std::chrono::high_resolution_clock::now();
  greator::cout << "Resizing took: "
                << std::chrono::duration<double>(stop - start).count() << "s"
                << std::endl;
}

template <typename T, typename TagT> void Index<T, TagT>::printTagToLocation() {
  std::shared_lock<std::shared_timed_mutex> lock(_tag_lock);
...
int Index<T, TagT>::insert_point(const T *point, const Parameters &parameters,
  ...
  if (location == -1) {
    ...
    if (_nd >= _max_points) {
      ...
      greator::cerr << "Thread: " << std::this_thread::get_id()
                    << ": Increasing _max_points from " << _max_points << " to "
                    << new_max_points << " _nd is: " << _nd << std::endl;
...
int Index<T, TagT>::lazy_delete(const tsl::robin_set<TagT> &tags,
  ...
  if ((_eager_done) && (!_data_compacted)) {
    greator::cout << "Eager delete requests were issued but data was not "
                     "compacted, cannot proceed with lazy_deletes"
                  << std::endl;
    return -2;
  }
...
template <typename T, typename TagT> bool Index<T, TagT>::hasIndexBeenSaved() {
  return _is_saved;
}
...
void Index<T, TagT>::print_delete_set() const {
  greator::cout << "Delete set is of size: " << _delete_set.size() << std::endl;
  ...
  greator::cout << "Sorted Delete set is of size: " << _delete_set.size()
                << std::endl;
  ...
  greator::cout << "Missing ids in delete set for the 60k-80k case:"
                << std::endl;
...
void Index<T, TagT>::are_deleted_points_in_graph() const {
  ...
  if (start_end_pairs.size() > 0) {
    greator::cout << "Found " << start_end_pairs.size()
    ...
    greator::cout << "Min source id: " << start_end_pairs[0].first
                  << " Max source id: "
    ...
    greator::cout << "Min target id: " << start_end_pairs[0].second
                  << " Max target id: "
...
template <typename T, typename TagT> void Index<T, TagT>::print_status() const {
  greator::cout << "------------------- Index object: " << (uint64_t)this
                << " -------------------" << std::endl;
  greator::cout << "Number of points: " << _nd << std::endl;
  greator::cout << "Graph size: " << _final_graph.size() << std::endl;
  greator::cout << "Location to tag size: " << _location_to_tag.size()
                << std::endl;
  greator::cout << "Tag to location size: " << _tag_to_location.size()
                << std::endl;
  greator::cout << "Number of empty slots: " << _empty_slots.size()
                << std::endl;
  greator::cout << std::boolalpha << "Data compacted: " << this->_data_compacted

    ...
    if (best_L_nodes[k].flag) {
      ...
      for (unsigned m = 0; m < des.size(); ++m) {
        ...
        if (inserted_into_pool.find(id) == inserted_into_pool.end()) {
          ...
          if ((m + 1) < des.size()) {
            auto nextn = des[m + 1];
            diskann::prefetch_vector((const char *)_data +
      ...
      if (tag_filename == nullptr) {
        ...
        if (file_exists(tag_filename)) {
          ...
          diskann::cerr << "Tag file " << tag_filename
                        << " does not exist. Exiting..." << std::endl;
          throw diskann::ANNException(std::string("Tag file") + tag_filename +
                                          " does not exist",
                                      -1, __FUNCSIG__, __FILE__, __LINE__);
        }
      }
...
template <typename T, typename TagT> void Index<T, TagT>::print_status() const {
  ...
  diskann::cout << "Number of empty slots: " << _empty_slots.size()
                << std::endl;
  diskann::cout << std::boolalpha << "Data compacted: " << this->_data_compacted

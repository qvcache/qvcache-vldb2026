...
void WindowsAlignedFileReader::register_thread() {
  ...
  if (this->ctx_map.find(std::this_thread::get_id()) != ctx_map.end()) {
    greator::cout << "Warning:: Duplicate registration for thread_id : "
  ...
  if (ctx.fhandle == INVALID_HANDLE_VALUE) {
    greator::cout << "Error opening " << m_filename.c_str()
...
IOContext &WindowsAlignedFileReader::get_ctx() {
  ...
  if (ctx_map.find(std::this_thread::get_id()) == ctx_map.end()) {
    ...
    throw greator::ANNException(stream.str(), -2, __FUNCSIG__, __FILE__,
                                __LINE__);
  }
    ...
    for (auto &os : ctx.reqs) {
      ...
      /*
        if (ResetEvent(os.hEvent) == 0) {
          greator::cerr << "ResetEvent failed" << std::endl;
          exit(-3);
        }
      */
    }
    ...
    for (_u64 j = 0; j < batch_size; j++) {
      ...
      if (ret == FALSE) {
        ...
        if (error != ERROR_IO_PENDING) {
          greator::cerr << "Error queuing IO -- " << error << "\n";
        }
      } else {
        greator::cerr << "Error queueing IO -- ReadFile returned TRUE"
                      << std::endl;
      }
    }
    DWORD n_read = 0;
      ...
      if (GetQueuedCompletionStatus(ctx.iocp, &n_read, &completion_key, &lp_os,
        ...
        if (lp_os == NULL) {
          ...
          if (error != WAIT_TIMEOUT) {
            greator::cerr << "GetQueuedCompletionStatus() failed with error = "
                          << error << std::endl;
